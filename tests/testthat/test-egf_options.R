test_that("egf_model", {
  ...
  expect_vector(x$curve, character(), 1L)
  expect_true(x$curve %in% eval(formals(egf_model)$curve))

  expect_vector(x$excess, logical(), 1L)
  expect_false(is.na(x$excess))

  expect_vector(x$family, character(), 1L)
  expect_true(x$family %in% eval(formals(egf_model)$family))

  expect_vector(x$day_of_week, integer(), 1L)
...
test_that("egf_control", {
  ...
  expect_vector(x$trace, integer(), 1L)
  expect_true(x$trace %in% 0:2)

  expect_vector(x$profile, logical(), 1L)
  expect_false(is.na(x$profile))

  expect_vector(x$sparse_X, logical(), 1L)
  expect_false(is.na(x$sparse_X))

  expect_vector(x$omp_num_threads, integer(), 1L)
...
test_that("egf_inner_optimizer", {
  ...
  expect_vector(x$method, character(), 1L)
...
test_that("egf_parallel", {
  ...
  expect_vector(x$outfile, character(), 1L)
  expect_false(is.na(x$outfile))

  expect_vector(x$cores, integer(), 1L)
