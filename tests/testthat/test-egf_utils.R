...
test_that("egf_sanitize_formula_parameters", {
  model <- egf_model(curve = "exponential", excess = FALSE, family = "pois", day_of_week = FALSE)
...
test_that("egf_make_frame", {
  ...
  o1 <- res$ts
  o1_expected <- data.frame(
    ts = gl(2L, 10L, labels = 2:3),
    window = factor(rep.int(c(NA, 1, 2, NA, 3, NA), c(1L, 4L, 5L, 1L, 4L, 5L)), labels = sprintf("window_%d", 1:3)),
...
test_that("egf_make_priors", {
  ...
  expect_identical(priors$bottom, list(beta = list(p1, p1, p1, p1), theta = list(p1, f(1L), f(2L), p1, NULL, p3), Sigma = list(p4)))
})
...
test_that("egf_make_Z", {
  ...
  Z2 <- Matrix::sparseMatrix(
    i = seq_len(10L),
    j = rep.int(seq_len(6L), c(2L, 2L, 1L, 1L, 2L, 2L)),
    x = data$x,
    dims = c(10L, 6L),
    dimnames = list(as.character(seq_len(10L)), sprintf("(x | %s)", c("f1:g1", "f2:g1", "f3:g1", "f3:g2", "f4:g2", "f5:g2")))
  )
...
test_that("egf_combine_X", {
  ...
  l <- egf_combine_X(fixed = fixed, X = lapply(fixed, egf_make_X, data = data, sparse = FALSE))
...
test_that("egf_combine_Z", {
  ...
  l <- egf_combine_Z(random = random, Z = lapply(random, egf_make_Z, data = data))
  ...
  Z <- as(Z, "dgCMatrix")
  effects <- data.frame(
    cov = `levels<-`(interaction(l$effects[c("term", "group")], drop = TRUE, lex.order = TRUE), disambiguate(rep.int("Sigma", 4L))),
    vec = `levels<-`(interaction(l$effects[c("term", "group", "level")], drop = TRUE, lex.order = TRUE), disambiguate(rep.int("u", 10L))),
