...
test_that("egf_combine_Z", {
  ...
  expect_equal(info$cov, `levels<-`(interaction(info[c("term", "group")], drop = TRUE, lex.order = TRUE), enum_dupl_string(rep_len("Sigma", 4L))))
  ...
  expect_equal(do.call(order, unname(info[c("cov", "vec", "top")])), seq_len(10L))
})
