...
test_that("d(inv)?wishart", {
  ...
  res1 <- get_test_res(test_enum = "dwishart", x = x, df = df, scale = scale, give_log = 1L)
  expect_equal(res1, dwishart(x, df, scale, TRUE))
  res2 <- get_test_res(test_enum = "dinvwishart", x = x, df = df, scale = scale, give_log = 1L)
...
test_that("dpois_robust", {
  ...
  res <- get_test_res(test_enum = "dpois_robust", x = x, log_lambda = log_lambda, give_log = TRUE)
...
test_that("eval_log_curve_exponential", {
  ...
  res <- get_test_res(test_enum = "eval_log_curve_exponential", time = time, log_r = log(r), log_c0 = log(c0))
...
test_that("eval_log_(curve|rt)_subexponential", {
  ...
  res1 <- get_test_res(test_enum = "eval_log_curve_subexponential", time = time, log_alpha = log(alpha), log_c0 = log(c0), logit_p = qlogis(p))
  expect_equal(res1, log(c0) + log1p((1 - p) * alpha * time / c0^(1 - p)) / (1 - p))

  res2 <- get_test_res(test_enum = "eval_log_rt_subexponential", log_curve = res1, log_alpha = log(alpha), logit_p = qlogis(p))
...
test_that("eval_log_(curve|rt)_gompertz", {
  ...
  res1 <- get_test_res(test_enum = "eval_log_curve_gompertz", time = time, log_alpha = log(alpha), log_tinfl = log(tinfl), log_K = log(K))
  expect_equal(res1, log(K) - exp(-alpha * (time - tinfl)))

  res2 <- get_test_res(test_enum = "eval_log_rt_gompertz", log_curve = res1, log_alpha = log(alpha), log_K = log(K))
...
test_that("eval_log_(curve|rt)_logistic", {
  ...
  res1 <- get_test_res(test_enum = "eval_log_curve_logistic", time = time, log_r = log(r), log_tinfl = log(tinfl), log_K = log(K))
  expect_equal(res1, log(K) - log1p(exp(-r * (time - tinfl))))

  res2 <- get_test_res(test_enum = "eval_log_rt_logistic", log_curve = res1, log_r = log(r), log_K = log(K))
...
test_that("eval_log_(curve|rt)_richards", {
  ...
  res1 <- get_test_res(test_enum = "eval_log_curve_richards", time = time, log_r = log(r), log_tinfl = log(tinfl), log_K = log(K), log_a = log(a))
  expect_equal(res1, log(K) - log1p(a * exp(-a * r * (time - tinfl))) / a)

  res2 <- get_test_res(test_enum = "eval_log_rt_richards", log_curve = res1, log_r = log(r), log_K = log(K), log_a = log(a))
...
test_that("logspace_add_(baseline|offsets)", {
  ...
  log_b <- log(2)
  res1 <- get_test_res(test_enum = "logspace_add_baseline", log_curve = log_curve, time = time, log_b = log_b)
  expect_equal(res1, log(exp(log_b) * time + exp(log_curve)))

  log_w <- log(c(1, 0.9, 1.1, 0.8, 1.2, 0.7, 1.3))
  from <- 5L
  res2 <- get_test_res(test_enum = "logspace_add_offsets", log_diff_curve = log_diff_curve, log_w = log_w, from = from)
