test_that("object", {
  ...
  expect_length(po, 6L)
  expect_named(po, c("top", "ts", "window", "linear_combination", "value", "deviance"), ignore.order = FALSE)
  ...
  expect_identical(c(by(po, po$linear_combination, f)), co[1:2], ignore_attr = "names")
})
...
test_that("parallel", {
  ...
  po_multicore <- profile(o, subset = (country == "A" & wave == 1),
  ...
  po_snow <- profile(o, subset = (country == "A" & wave == 1),
