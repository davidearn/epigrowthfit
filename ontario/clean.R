## Load
d_full <- read.csv(file = "2021-03-15/IPHIS_REPORT_2years.CSV")
...
## Clean and save
ord <- do.call(order, on_by_region[c("region", "date")])
on_by_region <- on_by_region[ord, , drop = FALSE]
...
ord_al <- order(as.numeric(sub("^([0-9]{1,3})-.*$", "\\1", al)))
on_by_age$age <- ordered(on_by_age$age, levels = al[ord_al])
ord <- do.call(order, on_by_age[c("age", "date")])
on_by_age <- on_by_age[ord, , drop = FALSE]
