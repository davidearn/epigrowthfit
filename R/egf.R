...
#' @param model
#'   An \code{"\link{egf_model}"} object specifying a nonlinear model and
#'   an observation model to be estimated.
#' @param control
#'   An \code{"\link{egf_control}"} object specifying control parameters.
#' @param do_fit
#'   A \link{logical} scalar. If \code{TRUE}, then \code{egf} returns early
#'   with a \link{list} of optimization inputs.
#' @param se
#'   A \link{logical} scalar. If \code{TRUE}, then standard errors on mixed
...
#' @param excess
#'   A \link{logical} scalar. If \code{TRUE}, then a constant baseline
...
#' @param day_of_week
#'   An integer. If positive, then day of week effects are estimated as
...
#'
#' Set parameters controlling the behavior of \code{\link{egf}}.
#'
#' @param optimizer
...
#'   the listed methods are tried in order until one succeeds, and a warning
#'   is issued if none succeed.
#' @param trace
#'   An integer determining the amount of tracing performed (see Details).
#'   \link[=logical]{Logical} values are coerced to integer.
#' @param profile
#'   A \link{logical} scalar. If \code{TRUE}, then fixed effect parameters
#'   are profiled out of the likelihood, which may stabilize optimization
#'   for models with many fixed effects. This feature is very experimental,
#'   as it relies on assumptions about the optimization problem that most
#'   likely do not hold for the models fit by \code{\link{egf}}.
#' @param sparse_X
#'   A \link{logical} scalar. If \code{TRUE}, then the fixed effects
...
#'   current response matrix \code{Y}. \code{Y[i, j]} is the
#'   current value of nonlinear or dispersion model parameter
#'   \code{j} (link scale) in fitting window \code{i}.
#' }
#' }
#'
...
#' evaluation; and
#' \item trace flags set by [TMB::config()] are turned on.
#' }
#'
#' @return
...
#' See \code{\link[TMB]{MakeADFun}} for (some) details about outer and inner
#' optimizations.
#'
#' @param f
...
#' }
#' For details, see the handling of \code{inner.method} and
#' \code{inner.control} in the body of \code{\link[TMB]{MakeADFun}}.
#'
#' @name egf_optimizer
NULL
