...
#'   from the end of date `start[i]` to the end of date `end[i]` in
#'   time series `ts[i]`.
#' @param window
#'   A factor generated by `make_window(, ts, )`.
#'
#' @details
#' `make_window(date, ts, endpoints)` constructs a factor `window`
#' such that `split(date, window)` splits the time points in `date`
#' by fitting window.
#'
...
#'
#' Missing values in `date` and `ts` are an error.
#' Rows of `endpoints` containing missing values are discarded silently.
#' Window endpoints not enclosing at least two time points are an error.
#'
#' @return
#' A factor of length `length(date)`. For `make_window()`, there are
...
make_window <- function(date, ts, endpoints) {
  stop_if_not(
    inherits(date, "Date"),
  ...
  stop_if_not(
    length(date) == length(ts),
    m = "`date` and `ts` must have equal length."
  )
  stop_if_not(
    !anyNA(date),
    !anyNA(ts),
  ...
  make_segment <- function(date, endpoints, prev) {
    ...
    w <- rep.int(NA_integer_, length(date))
    w[unlist(index)] <- rep.int(prev + seq_along(index), lengths(index))
    w
  }

  window <- rep.int(NA_integer_, length(date))
  split(window, ts) <-
    Map(make_segment,
      date = split(date, ts),
...
make_Z <- function(x, frame) {
  ...
  g <- interaction(frame[gv], drop = TRUE, sep = ":", lex.order = TRUE)
...
make_tmb_data <- function(frame_ts, frame_glmm,
  ...
  Z_info$cor <- interaction(Z_info[c("term", "group")], drop = TRUE, lex.order = TRUE)
