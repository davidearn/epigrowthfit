...
#'   in the case of \code{eval_append},
...
#' eval_subset(subset, data)
#'
#' order <- quote(order(month, day))
#' eval_order(order, data)
#'
#' append <- quote(-day)
#' eval_append(append, data)
#'
#' label <- quote(sprintf("%d-%02d-%02d", year, match(month, month.abb, 0L), day))
#' eval_label(label, data)
#'
#' @noRd
NULL

eval_subset <- function(expr, data, enclos = parent.frame()) {
...
eval_order <- function(expr, data, enclos = parent.frame()) {
  stopifnot(is.data.frame(data))
  n <- nrow(data)
  if (is.null(expr)) {
    return(seq_len(n))
...
eval_append <- function(expr, data, enclos = parent.frame()) {
  stopifnot(is.data.frame(data))
  if (is.null(expr)) {
    return(integer(0L))
  }
...
eval_label <- function(expr, data, enclos = parent.frame()) {
  stopifnot(is.data.frame(data))
  if (is.null(expr)) {
    return(NULL)
  }
