...
\author{Mikael Jagan, Benjamin M.\ Bolker, David J.\ D.\ Earn,\\
  Jonathan Dushoff, Junling Ma}
\date{\today}
...
<<rtools-2, eval=FALSE>>=
write(c('PATH="C:\\Rtools\\bin;${PATH}"',
...
Under construction, but see \cref{sec:ontario-ci}.
...
together specifying an interval incidence time series---in this
case, the daily incidence time series in the \code{ontario}
data frame. First, some technical notes:
%
\begin{itemize}
  \item \code{date} can also be a character vector, provided
    that it is coercible to an increasing Date vector with
    \code{`9as.Date`9(date, tryFormats = dfmt)}, where \code{dfmt}
    is an optional character argument to \fun{smooth_cases}
    listing date formats to consider in the conversion.
    By default, \code{dfmt = `2"\%Y-\%m-\%d"`2}, allowing only
    strings of the form \code{`2"YYYY-MM-DD"`2}. Details about
    character-to-Date conversion can be found under \code{?strptime}.
  \item For \code{i > `11`1}, \code{cases[i]} must specify the
    number of cases observed between \code{date[i-`11`1]} and
    \code{date[i]}. \code{cases[`11`1]} is ignored.
  \item In general, \code{date} need not be equally spaced,
    and \code{cases} can contain missing values.
  \item The first two arguments of \fun{smooth_cases} are
    \code{formula} and \code{data}. When defined correctly
    (see example below and find details under \code{?smooth_cases}),
    these are used instead of \code{date} and \code{cases}.
    While convenient, the \code{formula} interface, if unused,
    requires \code{date} and \code{cases} to be named explicitly
    in the call to \fun{smooth_cases}.
\end{itemize}
%
We also pass a logical scalar \code{log} and a numeric scalar
\code{spar} telling \fun{smooth_cases} whether to fit
\code{`9log10`9(`11`1+cases)} instead of \code{cases} and what
value in the interval $(0,1]$ to use for the spline smoothing
parameter. Details about \code{spar} can be found under
\code{?spline.smooth}.

<<ontario-smooth-1>>=
## `date` / `cases` interface
sc <- smooth_cases(
  date = ontario$date,
...
## `formula` / `data` interface (same thing)
sc <- smooth_cases(
  formula = new_confirmed ~ date,
  data = ontario,
  log = TRUE,
  spar = 0.7
)
...
produces a reasonable fit to the $\log_{10}(1+x)$-transformed
Ontario data. In practice, obtaining a such a fit can require
some coarse tuning of \code{spar}. To update a \code{smooth_cases}
object with a new value for \code{spar},
...
\code{ontario$date[`177`1]} and the second not peaking
...
\fun{dline} conveniently rounds specified dates to the
nearest date in \code{date = ontario$date}, converts the
result to numeric user coordinates, then adds vertical lines
to the current \code{smooth_cases} plot at those coordinates.
...
Arguments \code{date} and \code{cases} of \fun{egf_init} supply
an interval incidence time series. They are subject to the same
constraints as the so-named arguments of \fun{smooth_cases},
and can likewise be omitted in favour of arguments \code{formula}
and \code{data} (see \cref{sec:ontario-smooth}).
...
model. \fun{egf_init} arguments \code{first} and \code{last} index
the left and right endpoints, so that the fitting window starts at
\code{date[first]} and ends at \code{date[last]}, and only the counts
\code{cases[(first+`11`1):last]} affect the fitted model.%
\footnote{
  \code{cases[first]} is excluded because it counts cases observed
  prior to \code{date[first]}, the start of the fitting window.
}
Alternatively, \code{first} and \code{last} can be Dates or characters
coercible to Date. In this case, conversion to index is accomplished
(roughly) as follows, where \code{dfmt} is an optional argument to
\fun{egf_init} (see again \cref{sec:ontario-smooth}).

<<ontario-init-1, eval=FALSE>>=
## Not run
if (is.character(first)) {
  first <- as.Date(first, tryFormats = dfmt)
}
first_index <- which.min(abs(date - first))
@
...
\footnote{
  ...
  it is typically better for the fitting window to end at or even
  before the time when growth becomes subexponential~\cite{Ma+14}.
}

Following \cref{fig:ontario-smooth-3}, we will use
\code{first = `127`1} and \code{last = `177`1}
...
it better to set \code{first} and \code{last} explicitly
based on prior inspection of the time series.

In general, \code{smooth_cases} plots greatly simplify
the task of defining reasonable fitting windows. Generating
a plot like \cref{fig:ontario-smooth-3} before constructing
a call to \fun{egf_init} is usually a good idea.
...
can be used to specify positive initial estimates for some or all
relevant model parameters, but is optional, as every parameter has
a reasonable, internally defined default.
...
\footnote{
  Here, ``intercept'' refers to the value of the linear model at the
  left endpoint of the fitting window, and ``cumulative incidence''
  refers to the number of cases observed since that time.
}
...
specified by assigning argument \code{peak} the index of \code{date}
corresponding to the time of the peak or, alternatively, a date
to be coerced to index (see \code{first} and \code{last} above).

Following \cref{fig:ontario-smooth-3}, we will use
\code{peak = `177`1} for the first wave in the Ontario data and
...
We are finally equipped to run \fun{egf_init}. We do this once
for each of the two waves in the Ontario time series, in both
cases letting \fun{egf_init} define \code{theta_init} for us.

<<ontario-init-2>>=
## `date` / `cases` interface (first wave)
init1 <- egf_init(
  date = ontario$date,
  cases = ontario$new_confirmed,
  curve = "logistic",
...
## `formula` / `data` interface (second wave)
init2 <- egf_init(
  formula = new_confirmed ~ date,
  data = ontario,
  curve = "logistic",
...
The output of each initialization is a list of class \code{egf_init}
with \Sexpr{length(init1)} elements containing information used by
\fun{egf} and methods for class \code{egf_init}.
...
@
%
Elements \code{first}, \code{last}, \code{curve}, \code{distr}, and
\code{include_baseline} are copies of the arguments of \fun{egf_init}.%
\footnote{
  If defined as Date or character in the \fun{egf_init} call,
  then \code{first} and \code{last} are converted to index
  before final inclusion in the \code{egf_init} object.
}
Element \code{data} is a data frame with three variables
storing the interval incidence time series supplied in the
function call:

\nolinenumbers\par\vspace{6pt}
\begin{tabular}{rl}
\code{date} & date \\
\code{time} & number of days (integer) since \code{date[first]} \\
\code{cases} & number of cases observed since the previous time point
\end{tabular}
\vspace{6pt}\par\linenumbers\noindent
%
where \code{date[first]} is the start of the fitting window.

<<ontario-init-4>>=
## Temporarily add list to search path
attach(init1, name = "tmp")

head(data)
with(data, identical(date, ontario$date))
with(data, identical(time, as.integer(date - date[first])))
with(data, identical(cases, ontario$new_confirmed))
@
%
Element \code{window} has the value \code{first:last} and so
indexes the rows of \code{data} relevant to the fitting window.
...
gives the log-transformed estimates. To retrieve
\code{theta_init} and \code{log_theta_init},
...
%
taking numeric arguments \code{time} and \code{theta}
(default is \code{theta_init}) that specify times in days since
\code{data$date[first]} and parameter values named as in
\code{theta_init}. \code{`9eval_cum_inc`9(time, theta)} evaluates
\emph{expected} cumulative incidence at \code{time} days since
\code{data$date[first]}, conditional on parameter vector
\code{theta}. As we are using a logistic model, this amounts to
evaluating the right-hand side of \cref{eq:logistic}.

The \fun{predict} method for class \code{egf_init} provides
a convenient interface to \fun{eval_cum_inc} when one desires
\code{theta = theta_init}.

<<ontario-init-7>>=
wt <- data$time[window]
wp <- predict(init1, time = wt)
@
%
\fun{predict} returns a data frame with three variables
storing time series of expected cumulative incidence and
expected interval incidence, conditional on \code{theta_init}:

\nolinenumbers\par\vspace{6pt}
\begin{tabular}{rl}
\code{time} & number of days (possibly fractional) since \code{refdate} \\
\code{cum_inc} & expected number of cases observed since \code{refdate} \\
\code{int_inc} & expected number of cases observed since the previous time point
\end{tabular}
\vspace{6pt}\par\linenumbers\noindent
%
where \code{refdate = data$date[first]} is the start of
the fitting window.

<<ontario-init-8>>=
head(wp)
attr(wp, "refdate")
with(wp, identical(time, wt))
with(wp, identical(cum_inc, eval_cum_inc(wt, theta = theta_init)))
with(wp, identical(int_inc, c(NA, diff(cum_inc))))
@
...
Conveniently, it also takes an \code{add} argument, allowing
you to plot multiple \code{egf_init} objects together.
...
@
%
Don't be alarmed if the curves in an \code{egf_init} plot fail to
fit the data, as in \cref{fig:ontario-init-plot-1}. Poor fits are
to be expected from crude initial estimates of model parameters.
An \code{egf_init} plot should be used mainly as a final check
that the fitting window chosen for each wave is reasonable.
...
as starting points in an optimization algorithm to obtain
superior estimates---more precisely, numerical approximations
of the maximum likelihood estimates $\vec{\wh{\theta}}_\text{mle}$
(see \cref{sec:mle}).

The first argument of \fun{egf}, \code{init}, expects an
\code{egf_init} object. Argument \code{method} lets you
...
Argument \code{na_action} indicates whether \fun{egf}
...
\code{`2"exclude"`2} (default). Finally, argument
...
arguments will be passed directly to the optimizer: \fun{nlminb},
\fun{nlm}, or \fun{optim}, depending on \code{method}.
Below, we pass \code{init1} and \code{init2}, in turn, to \fun{egf},
each time accepting the defaults for \code{method}, \code{na_action},
and \code{nbdisp_tol}.
...
\fun{egf} returns a list of class \code{egf} with \Sexpr{length(fit1)}
elements specifying the fitted model and other information used by
methods for class \code{egf}.
...
@
%
The list retains argument \code{method} and elements \code{data},
\code{window}, \code{first}, \code{last}, \code{curve}, \code{distr},
and \code{include_baseline} from the \code{egf_init} object.

Element \code{theta_fit} is a numeric vector listing \emph{fitted}
parameter estimates, and element \code{log_theta_fit} gives the
log-transformed estimates. To retrieve \code{theta_fit} and
\code{log_theta_fit}, use the \fun{coef} method for class \code{egf}.
...
@
%
As usual, $r$ is expressed in units per day and $\thalf$ in days.

Element \code{nll} is the negative log likelihood of
\code{log_theta_fit}.%
\footnote{
  Recall from \cref{sec:mle} that \pkg{epigrowthfit} defines the
  negative log likelihood as a function of log-transformed parameters.
}
Elements \code{nll_func} and \code{nll_grad} are closures taking a
single numeric argument \code{log_theta}
(default is \code{log_theta_fit}) specifying log-transformed parameter
values in the order used by \code{log_theta_fit}. They evaluate the
negative log likelihood function and its gradient with respect to the
log-transformed parameters at \code{log_theta}.

<<ontario-fit-4>>=
## Temporarily add list to search path
attach(fit1, name = "tmp")
...
small perturbations of \code{log_theta_fit} have lower likelihood
(greater negative log likelihood). This provides some assurance
that \code{log_theta_fit} approximates a local maximum of the
likelihood surface. It does \emph{not} rule out the possibility
of multiple local maxima. Hence \code{log_theta_fit} may still
not approximate the global maximum, though such apprehensions are
typically resolved by plotting the model to assess goodness of
...
head(nll_ptb)
sum(nll_ptb > nll) / n # should be very close to 1
@

Element \code{eval_cum_inc} of the \code{egf} object is
a closure identical to that found in the \code{egf_init} object
(see \cref{sec:ontario-init}), except that the default value of
argument \code{theta} is \code{theta_fit}, not \code{theta_init}.
...
<<ontario-fit-6>>=
wt <- data$time[window]
wp <- predict(fit1, time = wt)
@
%
The output has the same structure as that of the method for class
\code{egf_init} (see again \cref{sec:ontario-init}).
...
@
%
Beyond the fact that it provides more information, one reason to
prefer the \fun{predict} method over \fun{eval_cum_inc} is that
it warns when you pass times outside of the fitting window, where
the fitted model may not be predictive.

<<ontario-fit-8>>=
predict(fit1, time = data$time[first] - 3:1)
predict(fit1, time = data$time[last] + 1:3)
@
...
(\fun{nlminb}, \fun{nlm}, or \fun{optim}). These can help an
experienced user diagnose poor fits, as well as warnings and
errors issued by the optimizer. \code{madf_out} is further
used by the \fun{confint} method for class \code{egf}
in order to compute confidence intervals on parameter estimates
(see \cref{sec:ontario-ci}). For details about \code{madf_out}
or \code{optim_out}, consult the relevant documentation,
\eg by running \code{?TMB::MakeADFun}.

Element \code{large_nbdisp_flag}, present only if
\code{distr = `2"nbinom`2"} as in this example, is a logical
scalar indicating whether $k$ in the fitted model
(\ie \code{theta_fit[[`2"nbdisp"`2]]}) exceeds the threshold
defined by \fun{egf} argument \code{nbdisp_tol} (see above).
...
allowing you to repeat the call with different arguments
using \pkg{stats} function \fun{update}. For example, to
check that a different optimization algorithm produces a
similar fit to your data, you can do:

<<ontario-fit-10>>=
fit1_redo <- update(fit1, method = "Nelder-Mead")

## Relative difference in parameter vector
...
\label{sec:ontario-ci}

Under construction. For now, see below and take a look at
\code{?confint.egf}.

<<ontario-ci-1>>=
confint(fit1, level = 0.95, trace = FALSE)
confint(fit2, level = 0.95, trace = FALSE)
@
...
should be interpreted with care. The displayed curve is supported
on a grid of equally spaced time points, and the spacing chosen
is the median observation interval. However, the displayed data may
not be equally spaced. This is not the case in the Ontario data,
which \emph{are} equally spaced, but it is the case in the Alberta
data, in which most observations are counts over one day, but some
are counts over longer intervals.
...
(1 day in the Alberta data) will tend to deviate from the plotted
curve not due to chance, but due to scale. These exceptional counts
should be noted when visually assessing whether the plotted curve
fits the data. The \fun{plot} method for classes \code{egf_init}
and \code{egf} highlight exceptional counts according to optional
argument \code{tol}, though this feature is not demonstrated in
\cref{fig:ontario-init-plot-1} or \cref{fig:ontario-fit-plot-1},
which display the equally spaced Ontario data.


\subsubsection{Customizing plots}

The \fun{plot} methods for classes \code{egf_init} and \code{egf}
take an optional argument \code{style} allowing for easy customization
...
plot(fit1,
  ...
  style = list(
    box = list(bty = "o"),
    main = list(line = 0.2, adj = 0),
    xax = list(mgp2 = c(0.25, 2), col.axis = c("black", "red")),
    window = NULL,
    points_main = list(pch = 18, col = "plum", cex = 1.5),
    lines = list(lty = 2, lwd = 4),
    text_dbl = list(
      x = "2020-04-14", y = 10, pos = 2, offset = 0,
...
To perform simulations, use the \fun{simulate} method for class
\code{egf}. The first argument, \code{object}, expects an \code{egf}
object. Arguments \code{nsim} and \code{seed} specify a number
of simulations and a seed for random number generation, used to
make simulations reproducible. You can specify time points as
numbers of days since the start of the fitting window,
\code{refdate = `9with`9(object, data$date[first])},
with optional argument \code{time}.
However, the default \code{time = `9with`9(object, data$time[window])}
is typically desirable, as it matches the time points in the
simulations to the time points in your data (restricted to the
fitting window).

<<ontario-fit-simulate-1>>=
ws <- simulate(fit1, nsim = 6, seed = 1257)
@

\fun{simulate} returns a list of class \code{egf_sim} with
\code{refdate} as an attribute. The list retains argument
\code{time} and includes additional elements \code{cum_inc}
and \code{int_inc}, both matrices with \code{`9length`9(time)}
rows and \code{nsim} columns storing simulations.

<<ontario-fit-simulate-2>>=
t(sapply(ws[-1], dim)) # dimensions of each matrix
@
...
distribution (depending on \code{object$distr}, in this example
\code{`2"nbinom"`2}) with mean equal to
\code{`9predict`9(object, time)$int_inc[i]}.
The negative binomial dispersion parameter, if used, takes its
fitted value \code{object$theta_fit[[`2"nbdisp"`2]]}.

\code{cum_inc[i, j]} is the number of cases observed up to
\code{time[i]} in simulation \code{j}. Column vector
\code{cum_inc[, j]} is computed as
\code{c0 + `9cumsum`9(`9c`9(`10`1, int_inc[-`11`1, j]))},
where \code{c0 = `9predict`9(object, time)$cum_inc[`11`1]}
is the value of expected cumulative incidence at \code{time[`11`1]}.

<<ontario-fit-simulate-3>>=
lapply(ws, head)
@

The \fun{plot} method for class \code{egf_sim} plots the predicted
and simulated time series. Optional arguments \code{col_pred} and
\code{col_sim} can be used to set colours, which may be desirable
when the number of simulations is large and greater transparency
is required to properly visualize their distribution
(see function \fun{alpha} in the \cran{scales} package).
...
par(mfrow = c(1, 2))
plot(ws, inc = "cumulative")
plot(ws, inc = "interval")
@
...
make use of the \code{egf} objects obtained in
\cref{sec:ontario-fit}, namely \code{fit1} and
\code{fit2}. These define our estimates of $r$ for
the first and second waves in the Ontario time series.
...
argument \code{r}, which can be a numeric vector listing initial
exponential growth rates $r$ in units day$^{-1}$, an \code{egf_init}
object, or an \code{egf} object. Numeric vectors \code{r} are
handled by the default method, which returns a numeric vector listing
doubling times in days corresponding to the elements of \code{r}
(see \cref{sec:doubling-time}). If \code{r} is an \code{egf_init}
or \code{egf} object, then the default method is applied to the
estimate of $r$ given by \code{r$theta_init[[`2"r"`2]]}
or \code{r$theta_fit[[`2"r"`2]]}, respectively.

<<ontario-doubling-time-1>>=
compute_doubling_time(10^(-2:0)) # default method
...
The default method takes numeric arguments \code{r}, \code{breaks},
and \code{probs} specifying $r$ in units day$^{-1}$, $\tau_i$ in days,
and $p_i$, respectively. It is vectorized in \code{r}, returning a
numeric vector of equal length listing corresponding values of $\R_0$.
Alternatively, \code{r} can be an \code{egf_init} or \code{egf} object.
In this case, the default method is applied to
\code{r$theta_init[[`2"r"`2]]} or \code{r$theta_fit[[`2"r"`2]]}, respectively. \code{probs} must have length
\code{`9length`9(breaks)-`11`1}, and \code{probs[i]}
should define the probability that the generation interval
is between \code{breaks[i]} and \code{breaks[i+1]} days.%
\footnote{
  It is actually sufficient for \code{probs} to give probability
  \emph{weights}, because \fun{compute_R0} normalizes for you,
  \ie it assigns \code{probs <- probs / `9sum`9(probs)} internally.
}

For the purpose of demonstrating \fun{compute_R0} usage,
we assume that the generation interval in days follows the
...
\pkg{epigrowthfit} data set \code{covid_generation_interval}
supplies the parameters of this distribution,
called \code{shape} and \code{scale}, and values for \code{breaks}
and \code{probs} that can be passed directly to \fun{compute_R0}.
