...
in \cref{sec:ontario} demonstrates, where it is applied to
a COVID-19 data set from Ontario, Canada.
...
the number of points to which an epidemic model is fit,
...
but as it makes no use of gradients, it is relatively slow. Speed
may not be a concern in practice, as the models implemented in
...
transmission). One can show that if $f_\text{lat}$ and
$f_\text{inf}$ have discrete support, then $f_\text{gen}$
satisfies the conditions under which \cref{eq:R0} is valid.
\pkg{epigrowthfit} functions \fun{dgi} and \fun{pgi} evaluate
the density and distribution functions of $\tau_\text{gen}$
implied by distributions of $\tau_\text{lat}$ and
$\tau_\text{inf}$ supported on the positive integers
(\ie $1,2,\ldots$ days).  These are helpful if one wants to
use \fun{compute_R0} and has latent and infectious period
distributions, but not the corresponding generation interval
distribution.
...
clearly defined. \pkg{epigrowthfit} function \fun{smooth_cases}
...
example, the daily incidence time series in the \code{ontario}
data frame).%
\footnote{
...
produces a reasonable fit to the $\log(1+x)$-transformed Ontario
data. In practice, obtaining a such a fit can require a coarse
tuning of \code{spar}. To update the \code{smooth_cases} object,
one can do \code{sc <- `9update`9(sc, spar = new_value)}.
...
An optional third argument \code{na_action} indicates whether
\fun{egf} should throw an error if it finds missing values in
the fitting window, or if it should ignore them when calculating
likelihood (see \cref{sec:mle}). The options are \code{`2"fail"`2}
and \code{`2"exclude"`2} (default).
...
Further optional arguments will be passed directly to \fun{nlminb},
\fun{nlm}, or \fun{optim}, depending on \code{method}.
Below, we pass \code{init1} and \code{init2}, in turn,
to \fun{egf}, each time accepting the defaults for
...
one can check whether a different optimization algorithm
arrives at a similar fit as follows.
...
Plots of interval incidence by \fun{plot.egf_init} and \fun{plot.egf}
...
equally spaced. This is not the case in the Ontario data, which are
equally spaced, but it is the case in the Alberta data, in which most
observations are counts over one day, but some are counts over longer
intervals.
...
according to optional argument \code{tol} (run \code{?plot.egf_init}
for details), though this feature is not demonstrated in
\cref{fig:ontario-init-plot-1}, where the points are in fact
equally spaced.
...
time points in the fitting window.
