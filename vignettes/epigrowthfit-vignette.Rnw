...
\href{https://www.r-project.org/}{R} Version 3.3.0 or greater
...
from source R packages that use C++ code, and \pkg{epigrowthfit}
is such a package. Once installed, Rtools must be added to
the search path. Users with Rtools 40%
\footnote{As of October 20, 2020, Rtools 40 is the newest version
of Rtools, compatible with R Version 4.0.0 or greater.}
...
\code{C:\\Rtools}. Users with one of these versions can run
...
@
%
replacing the substring \code{C:\\\\Rtools} if necessary.
Finally, at least one Windows user running Rtools 40 has
encountered the following install-time error:

<<rtools-3, eval=FALSE, highlight=FALSE>>=
C:/Rtools/mingw_64/bin/g++: No such file or directory
@
%
Compiler \code{g++} was not found because \code{C:\\Rtools}
was not the path to Rtools 40. The error was resolved after
running
...
@
%
and restarting R, as this specified the correct path to the
compiler.
...
the root directory, namely \code{epigrowthfit/}. This
will install \pkg{epigrowthfit} and its dependencies
and build this vignette. You can verify that the installation
...
Let $c(t)$ be the expected number of cases observed between the start
of an epidemic wave%
...
and $t$ days later, and let $c(0) = c_0 > 0$. $c(t)$ will be called
expected ``cumulative incidence'', with the understanding that it
does not necessarily count cases since the very start of an epidemic.
...
It is worth noting that the negative binomial distribution with
...
model given by \cref{eq:logistic} becomes
%
\begin{linenomath}
\begin{equation}
...
(\eg an epidemic occurring over less that one year), over which the
mortality rate is roughly constant. On longer time scales, more
sophisticated models of baseline mortality involving multiple
...
\cran{TMB} to carry out automatic differentiation of $-\ell$ with
...
\code{?canadacovid}. You can also directly examine the
...
not critical, but a reasonable initial guess is important for
convergence. Reasonable initial estimates of $r$
...
cases observed since the start of the fitting window, and ``intercept''
refers to the value of the linear model at the start of the fitting
window.}
If the interval incidence curve is roughly symmetric, then an estimate
of $\thalf$ (\cref{eq:logistic,eq:richards}) is supplied by the time
between the start of the fitting window and the curve's peak. Similarly,
an estimate of $K$ (\cref{eq:logistic,eq:richards}) is supplied by
2 times the number of cases observed in this interval. When fitting a
Richards model (\cref{eq:richards}), one can initially assume logistic
growth by estimating that $p$ is 1. When including baseline growth in
a model (see \cref{sec:models-baseline}), one can use the average
mortality rate in the years preceding and following the epidemic as an
...
Optional \fun{egf_init} argument \code{theta0} can be set explicitly
to override any of these defaults. For details, run \code{?egf_init}.
...
$t_{i-1}$ and $t_i$ for $i = 1,\ldots,n$ (\ie interval incidence).
...
relies on the definition of free variables \code{theta0}, \code{curve},
and \code{include_baseline}, which can be seen by printing
...
value of \break\code{object$`9cum_inc`9(time, theta = object$theta0)},
...
This is convenient, especially when the fitting window selected
by \fun{egf_init} is not to your satisfaction, and you want to
run \fun{egf_init} again with a few arguments modified. For details,
run \code{?update}.
...
of argument \code{theta} is \code{theta_hat}, not \code{theta0}.
When \code{theta = theta_hat} is desired, use the \fun{predict}
...
\code{x$theta0[[`2"r"`2]]}. If \code{x} is an \code{egf} object,
then it applies the method for numeric vectors to the \emph{fitted}
value of $r$, namely \code{x$theta_hat[[`2"r"`2]]}.
