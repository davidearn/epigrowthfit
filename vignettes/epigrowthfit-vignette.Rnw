...
models of epidemic growth. Many of the remaining functions enable
estimation of parameters that are not explicitly fitted, from fitted initial exponential growth rates $r$. These include the
...
\cref{sec:ontario} demonstrates usage of the \pkg{epigrowthfit}
...
\pkg{epigrowthfit} tolerates missing values in the interval
incidence time series, except in the extreme case in which
the number of data points to which an epidemic model is fit,
excluding missing values, is less than the number of model
parameters (see \cref{sec:mle}). Hence, in practice, users
are not required to perform
\href{https://en.wikipedia.org/wiki/Imputation_(statistics)}{imputation}.
Nonetheless, it is a good idea to investigate likely causes
for missingness, as there may be straightforward ways to
eliminate missing values (see, \eg \cref{sec:data-weekends}).
...
Wednesday, November 11, 2020. Suppose that on day $i$, a number of
new cases since day $i-1$ is reported. For simplicity, suppose that
...
\footnote{The start of an epidemic wave is defined as the time at
...
half its final size, satisfying $c(\thalf) = \frac{K}{2}$.
This formulation is desirable, because, unlike $c_0$, $\thalf$ is
unconstrained: $c(t) \in (0,K)$ for all $t$,
...
type of data arises in the modeling of influenza. People who recover
from influenza are prone to later infection with pneumonia. Clinically,
pneumonia can be difficult to distinguish from influenza, and, as such,
many influenza deaths are attributed to pneumonia rather than influenza.
In this setting, influenza spread must be inferred from aggregates of
the pneumonia and influenza deaths.
...
\footnote{\cref{eq:logistic-baseline} models baseline mortality
as linear in time. This formulation is valid for short time scales
(\eg an epidemic occurring over less than one year), over which the
mortality rate is roughly constant. On longer time scales,
more sophisticated models of baseline mortality with more than one
...
between times $t_{i-1}$ and $t_i$ (interval incidence). The
primary task of \pkg{epigrowthfit} is to fit one of the epidemic
models defined in \cref{sec:models} to these data. More precisely,
...
$i$, then the likelihood function remains well-defined
if the product in \cref{eq:likelihood} is taken over
just the indices $i$ for which $x_i$ is known.
...
$\vec{\theta}^* = \log \vec{\theta}$:
%
\begin{linenomath}
\begin{equation}
-\ell^*(\vec{\theta}^*|\vec{x})
= -\ell(e^{\vec{\theta}^*}|\vec{x})
= -\ell(e^{\log \vec{\theta}}|\vec{x})
= -\ell(\vec{\theta}|\vec{x})\,,
\end{equation}
\end{linenomath}
%
...
of $-\ell^*$ with respect to $\vec{\wh{\theta}}^*$. It uses
...
$\vec{\wh{\theta}}_\text{mle}^*$. The optimizers are iterative
and susceptible to numerical problems preventing convergence near
to $\vec{\wh{\theta}}_\text{mle}^*$. Poor fits to the data
by \fun{nlminb} are sometimes corrected by trying a different
optimizer, such as the Nelder-Mead algorithm available through
\fun{optim}. Nelder-Mead is considered robust, but as it makes
no use of gradients, it is relatively slow. Speed may not be a
concern in practice, as the models implemented in \pkg{epigrowthfit}
have at most six parameters.
...
estimates of the initial exponential growth rate $r$ obtained
under the exponential model are highly sensitive to the choice of
fitting window. More robust and realistic fits to epidemic data
are obtained with the logistic and Richards models
(\cref{eq:logistic,eq:richards}), which allow $c(t)$ to saturate
asymptotically.
...
\end{linenomath}
%
(\cref{fig:models-observed} shows this graphically for $\mu = 20$.)
...
may seem natural to go with the more general negative binomial model.
It turns out that a more careful approach is needed:
...
\footnote{In an experiment, the computed likelihood of a
$\mathrm{NegativeBinomial}(1,k)$ model given 100 $\mathrm{Poisson}(1)$
deviates diverged from the computed $\mathrm{Poisson}(1)$ likelihood
starting near $k = 10^6$.}
For this reason, one should not rely on negative binomial fits
with large $k$. Instead, one should set a threshold value of $k$
...
\code{?canadacovid}. You can examine the data frame directly
as follows.
...
hence \code{date} and \code{new_confirmed} together specify an
interval incidence time series. The observation interval is fixed
and equal to 1 day, and there are no missing values.
(\pkg{epigrowthfit} tolerates unequal spacing and missing values,
but it is good practice nonetheless to inspect one's data in this
way for irregularities.)

<<ontario-data-4>>=
unique(diff(ontario$date))
sum(is.na(ontario$new_confirmed))
@
...
This will require us to know how many waves there are and what segments
of the date (which rows in the \code{ontario} data frame) should be used
when fitting. In a given wave, of specific interest are the times when
interval incidence starts and stops growing exponentially (linearly on a
logarithmic scale) and the time of peak incidence. Locating these times
requires some care, especially in noisy or unequally spaced time series,
for which notions of growth rate and peak are not clearly defined.
\pkg{epigrowthfit} function \fun{smooth_cases} attempts to simplify
this task by fitting a smooth cubic
...
together specifying our time series of daily incidence in Ontario.%
\footnote{For \code{i > `11`1}, \code{cases[i]} must specify
the number of cases observed between \code{date[i-`11`1]} and
\code{date[i]}. \code{cases[`11`1]} is ignored. The time series
need not be equally spaced, and missing values are tolerated.}
We also pass a logical \code{log} and numeric \code{spar} telling \fun{smooth_cases} whether to fit $\code{`9log10`9(`11`1+cases)}$
instead of \code{cases} and what value in the interval $(0,1]$
to use for the spline smoothing parameter.
...
produces a reasonable fit to the $log(1+x)$-transformed data.
In practice, obtaining a reasonable fit will likely require
iterative tuning of \code{spar}.

<<ontario-smooth-2, fig.height=3.5, fig.cap="Result of \\code{`9plot`9(sc)}. A cubic spline fit to daily incidence of COVID-19 in Ontario. Times of peaks and troughs in the fitted curve are listed in the top margin as integer indices of \\fun{smooth_cases} argument \\code{date = ontario$date}.">>=
plot(sc)
@
...
the end of the time series. Below, we use \pkg{epigrowthfit}
function \fun{dline} to mark roughly when daily incidence
starts and stops growing exponentially
(linearly on a logarithmic scale) in each of the two waves
(see \cref{fig:ontario-smooth-3}). \fun{dline} conveniently
rounds specified dates to the nearest date in
\code{date = ontario$date}, converts the result to numeric
user coordinates, then adds vertical lines to the current
\code{smooth_cases} plot.

<<ontario-smooth-3, fig.height=3.5, fig.cap="A duplicate of \\cref{fig:ontario-smooth-2}, but with exponential growth windows marked using \\fun{dline}.", echo=-1>>=
plot(sc)
...
is increasing roughly linearly, then interpolating by eye
the calendar on the bottom axis to find interval endpoints.
In practice, some iteration with \fun{dline} is typically
required to settle on reasonable dates.
...
model. \fun{egf_init} arguments \code{first} and \code{last} are used
to index the left and right endpoints in \code{date}. Hence the
fitting window starts at \code{date[first]} and ends at
\code{date[last]}, and we consider only \code{cases[(first+`11`1):last]}
when fitting. (Cases counted in \code{cases[first]} are observed prior
to \code{date[first]}.) Dates of the form \code{`2"YYYY-MM-DD"`2} can
be passed to \code{first} and \code{last} instead, in which case
conversion to index is accomplished internally with
\code{`9which.min`9(`9abs`9(date-`9as.Date`9(`2"YYYY-MM-DD`2")))}.

Ma \etal~\cite{Ma+14} examined the sensitivity of estimates of the
initial exponential growth rate $r$ and the associated
95\% confidence interval's coverage probability, to the endpoints
of the fitting window. A reasonable initial approach for the
logistic and Richards models, based on their analysis, is to start
at the time when interval incidence begins growing exponentially
(linearly on a logarithmic scale) and to end between when growth
becomes subexponential and when peak incidence is attained.%
\footnote{When fitting an exponential model, this approach results
in underestimation of $r$, because growth becomes subexponential
before the end of the fitting window. Hence, with an exponential
model, it is better for the fitting window to end at or before the
time when growth becomes subexponential~\cite{Ma+14}.}
...
for the second wave. In general, \code{smooth_cases}
plots greatly simplify the task of defining a reasonable
fitting window. Generating one of these plots before
constructing a call to \fun{egf_init} is recommended.
...
(\eg via a \code{smooth_cases} plot).
...
expects a named numeric vector, can be used to specify (positive)
initial estimates for some or all relevant model parameters,
but is optional, as every parameter
($r$, $c_0$, $\thalf$, $K$, $p$, $k$, and $b$)
has a reasonable, internally defined, default initial estimate.
...
only observations in the first half of the fitting window.
\footnote{Here, ``cumulative incidence'' refers to the number of
cases observed since the start of the window, and ``intercept''
refers to the value of the linear model at the start of the window.}
The default for $\thalf$ (\cref{eq:logistic,eq:richards}) is time
between the start of the fitting window and the peak in the focal wave.
The default for $K$ (\cref{eq:logistic,eq:richards}) 2 times the number
of cases observed in this interval. (The defaults for $\thalf$ and $K$
assume that the interval incidence curve is roughly symmetric about a
peak.) The default for $p$ (\cref{eq:richards}) is 1, as this value
reduces the Richards model to a logistic model. Finally, for simplicity,
the defaults for $k$ and $b$ are 1 and 1 day$^{-1}$, respectively.%
\footnote{A more reasonable initial estimate of $b$ is supplied by
the average mortality rate in the years preceding and following an
epidemic. This estimate can be specified by setting \code{thata_init}
explicitly, assuming \code{include_baseline = `1TRUE`1}.}
...
specified by assigning argument \code{peak} the appropriate index
of \code{date} or a date to be coerced to index (see \code{first}
and \code{last} above).
...
date (Octover 10, 2020) slightly later than the date when
growth becomes subexponential.
...
\code{curve}, and \code{include_baseline}, as can be seen by printing
\code{eval_cum_inc}. \fun{eval_cum_inc} retrieves these variables
from its enclosing environment, \code{`9environment`9(eval_cum_inc)},
which is the execution environment of its parent function \fun{egf_init}.}
%
taking numeric arguments \code{time} and \code{theta} (defaults
are elements \code{time} and \code{theta_init} of the \code{egf_init}
object) that specify times in days since \code{date[first]} and
parameter values named as in \code{theta_init}.
\code{`9eval_cum_inc`9(time, theta)} evaluates expected cumulative
incidence since \code{date[first]} side of \cref{eq:logistic} at
\code{time} days since \code{date[first]}, conditional on parameter
vector \code{theta}. That is, it simply evaluates the right-hand side
of \cref{eq:exponential,eq:logistic,eq:richards} (\cref{eq:logistic}
...
elements less than \code{object$time[object$first]} (zero) or
greater than \code{object$time[object$last+`11`1]},
where \code{object} is the \code{egf_init} object.

The \fun{predict} method for class \code{egf_init} provides a
convenient interface to \fun{eval_cum_inc}.

<<ontario-init-5>>=
predict(init1, time = 0:10)
@
%
...
namely \code{object$date[first]}. Element \code{cum_inc} is the
value of \code{object$`9cum_inc`9(time, theta = object$theta_init)},
and element \code{int_inc} is just
\code{`9c`9(`1NA`1, `9diff`9(cum_inc))},
...
the call to \fun{egf_init} that generated \fun{init1}:
...
case in the Alberta data, in which most observations are counts
over one day, but some are counts over more days.
...
according to optional argument \code{tol}, though there are no
exceptional counts in the Ontario data, so this highlighting feature
is not demonstrated in \cref{fig:ontario-init-plot-1}. For details,
run \code{?egf_init}.

The exact endpoints of the fitting windows are not obvious from
\cref{fig:ontario-init-plot-1}. The \fun{print} method for class
\code{egf_init} will tell you more about the window and remind
you that actually fitting a model requires a call to \fun{egf}
(see \cref{sec:ontario-fit}).
...
$\vec{\wh{\theta}}_\text{init}$ of the model parameter vector
$\vec{\theta}$ produces a poor fit to the \code{ontario} data.
Function \fun{egf} uses this initial guess as a starting point
in an optimization algorithm to obtain a superior estimate---%
more precisely, a numerical approximation of the maximum likelihood
estimate $\vec{\wh{\theta}}_\text{mle}$ (see \cref{sec:mle}).

The first argument of \fun{egf}, \code{init}, expects an
\code{egf_init} object. An optional second argument \code{method}
...
An optional third argument \code{na_action}, one of
\code{`2"fail"`2} and \code{`2"exclude"`2} (default),
indicates whether \fun{egf} should throw an error if
it finds missing values in the fitting window, or if
it should ignore them (see \cref{sec:mle}).
An optional fourth argument \code{nbdisp_tol} defines a threshold
$\kappa$ (default is 100) on the negative binomial dispersion
parameter $k$. If $k > \kappa \max_i \{\mu_i\}$ in the fitted
model, then \fun{egf} will issue a warning suggesting to refit
using a Poisson model (see \cref{sec:choosing}).
Further optional arguments will be passed directly to \fun{nlminb}, \fun{nlm}, or \fun{optim}, depending on \code{method}.
Below, we pass \code{init1} and \code{init2}, in turn, to
\fun{egf}, each time accepting the defaults for
\code{method}, \code{na_action}, and \code{nbdisp_tol}.
...
\end{enumerate}
%
These checks provide some assurance that \fun{log_theta_fit}
approximates a local maximum of the likelihood surface. They do not,
however, rule out the possibility of multiple local maxima. Hence
\code{theta_fit} may still not approximate the global maximum. Such
apprehensions are typically resolved by plotting the fitted model to
assess goodness of fit (see \cref{sec:ontario-fit-plot}).

<<ontario-fit-5>>=
## Check 1
norm(nll_grad(log_theta_fit))
...
and warnings and errors issued by the optimizer. For details,
...
Finally, element \code{call} is the call to \fun{egf},
allowing one to repeat the call with different arguments
(\eg an alternative value of \code{method}, specifying a
new optimization algorithm) using \pkg{stats} function
\fun{update}.
...
<<ontario-fit-plot-1, fig.height=7, fig.cap='Result of \\code{`9plot`9(fit, inc)}: \\textbf{[Top]} cumulative incidence (\\code{inc = `2"cumulative"`2}) and \\textbf{[Bottom]} interval incidence (\\code{inc = `2"interval"`2}). Points are observed incidence. Lines are expected incidence conditional on fitted parameter estimates. Incidence is plotted on a logarithmic scale, with zeros plotted directly on the horizontal axis. The shaded rectangle displays the selected fitting window.'>>=
par(mfrow = c(2, 1)) # create a two-panel plot
...
take a number of optional arguments allowing for easy customization
of various plot elements (\eg line type, width, and colour). For a
complete list of available options, run \code{?plot.egf}.
...
fitting process, and assess how the new fit differs from the original.
...
an element \code{refdate} storing reference date \code{object$init$date[`11`1]};
and elements \code{int_inc} and \code{cum_inc}, both matrices storing
simulations.
...
its fitted value \code{object$theta_fit[[`2"nbdisp"`2]]}.
...
compute_R0(init1, breaks, probs) # method for class "egf_init"
