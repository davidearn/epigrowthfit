...
(\eg an epidemic occurring over less that one year), over which
...
different fitting windows. Optional \fun{egf_init} arguments
\code{peak}, \code{last}, \code{first}, \code{wlen}, \code{min_wlen},
and \code{max_wlen} can be set explicitly to override the default
...
Element \code{nbdisp_flag}, present only if
...
complete list of available options, run \code{?`2"egf_init-methods"`2}
or \code{?`2"egf-methods"`2}.
