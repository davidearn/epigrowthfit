...
from source R packages R packages like \pkg{epigrowthfit} that
rely on compiled C++ code. Once installed, Rtools must be added
to \code{PATH}. Users with Rtools 40 (compatible with R 4.0.0
...
\subsubsection{Installing from the remote repository}
...
\label{eq:nbinom-pmf}
f_i(j;\vec{\xi},k)
= \frac{\Gamma(k + j)}{j! \Gamma(k)} \bigg(\frac{k}{k + \mu_i(\vec{\xi})}\bigg)^k \bigg(\frac{\mu_i(\xi)}{k + \mu_i(\xi)}\bigg)^j\,\qquad j = 0,1,\ldots
\end{equation}
\end{linenomath}
...
<<models-observed, fig.height=2.5, fig.cap=paste0("Probability mass functions of three negative binomial random variables ($k = ", paste0(k, collapse = ", "), "$) and a Poisson random variable with a common mean of ", mu, "."), echo=FALSE>>=
val <- 0:60
mu <- 20
k <- 10^{c(0:2)}
...
\footnote{\cref{eq:logistic-baseline} models baseline mortality as
linear in time. This formulation is valid for short time scales
(\eg an epidemic occurring over less than one year), over which
the mortality rate is roughly constant. On longer time scales,
more sophisticated models of baseline mortality with multiple
parameters can be considered. For an early example,
...
the set of random variables $\{X_i\}$ is mutually independent.
...
concern in practice, as the models implemented in
\pkg{epigrowthfit} have at most six parameters.
...
$k \gg \max_i \{\mu_i\}$ is prone to inaccuracies.%
...
We will fit a logistic model (see \cref{sec:models-expected}) with
...
based on their analysis, is to start at the time when interval
incidence begins to grow exponentially (\ie when the logarithm of
interval incidence becomes linear) and to end at the time of the
peak in interval incidence (equivalently, the inflection point in
cumulative incidence; see \cref{fig:data-types}). If the peak has
not occurred (\eg when fitting in real time, during a growing
epidemic), then end at the last observation time.
...
it is typically best for the window to end at the time when
interval incidence stops growing exponentially (\ie when the
logarithm of interval incidence stops being linear).

Regardless of the model being fit, one may need to experiment
with different fitting windows. Optional \fun{egf_init} arguments
\code{peak}, \code{last}, \code{first}, \code{min_first}, and
\code{max_first} can be set explicitly to override the default
window selection algorithm. In fact, setting these arguments
explicitly will be \textbf{necessary} if the interval incidence
time series given to \fun{egf_init} spans multiple epidemic waves.
If the user gives no instructions about which wave to fit, then
\fun{egf_init} is unlikely to select a reasonable fitting window.
For details about the optional \fun{egf_init} arguments,
run \code{?egf_init}.

\paragraph{Initial parameter estimates.} The optimization algorithms
employed by \pkg{epigrowthfit} (see \cref{sec:mle}) are iterative
and require an initial estimate $\vec{\wh{\theta}}_\text{init}$ of
parameter vector $\vec{\theta}$. The exact choice of
$\vec{\wh{\theta}}_\text{init}$ is typically not critical, but a
reasonable initial guess is important for convergence. Reasonable
initial estimates of $r$ (\cref{eq:exponential,eq:logistic,eq:richards})
...
only observations in the first half of the fitting window, where growth
is more likely to be exponential.%
\footnote{Here, cumulative incidence refers to the number of cases
observed since the start of the fitting window, and ``intercept''
refers to the value of the linear model at the start of the fitting
window.}
If the interval incidence curve is roughly symmetric about a peak,
then an estimate of $\thalf$ (\cref{eq:logistic,eq:richards}) is
supplied by the time between the start of the fitting window and the
peak. Similarly, an estimate of $K$ (\cref{eq:logistic,eq:richards})
is supplied by 2 times the number of cases observed in this interval.
When fitting a Richards model (\cref{eq:richards}), one can initially
assume logistic growth by estimating that $p$ is 1. When including
baseline growth in a model (see \cref{sec:models-baseline}), one can
use the average mortality rate in the years preceding and following
the epidemic as an initial estimate of $b$.

With the exception of $b$, \fun{egf_init} employs these initial
estimates by default. For simplicity, the default initial estimates
of parameters $b$ and $k$ are 1 day$^{-1}$ and 1, respectively.
Optional \fun{egf_init} argument \code{theta_init} can be set
explicitly to override any of these defaults. For details, run
\code{?egf_init}.
...
specifying the number $x_i$ of cases observed between times
$t_{i-1}$ and $t_i$ for $i = 1,\ldots,n$ (interval incidence).
We indicate the model we want to fit using these arguments:
...
estimates for us as described above. (The second wave of Ontario's
COVID-19 epidemic does not start until August 2020, after the
the last date in the \code{canadacovid} data set.)
...
window. Element \code{theta_init} is a numeric vector listing initial
parameter estimates, which in this example were all selected by
\fun{egf_init}. Element \code{log_theta_init} gives the log-transformed
estimates. To retrieve \code{theta_init} and \code{log_theta_init},
you can use the \fun{coef} method for class \code{egf_init}.

<<ontario-init-4>>=
coef(init) # same as `init$theta_init`
coef(init, log = TRUE) # same as `init$log_theta_init`
@
%
...
incidence at \code{time} days since \code{date[`11`1]} conditional
on parameter vector \code{theta}. Specifically, \fun{eval_cum_inc}
calculates the number of cases observed up to the start of the
fitting window, and adds to this the expected number of cases
observed between the start of the fitting window and \code{time},
which it computes using \cref{eq:exponential,eq:logistic,eq:richards}
(\cref{eq:richards} in this example).%
\footnote{Recall from \cref{sec:models-expected}
that \cref{eq:exponential,eq:logistic,eq:richards} model the expected
number of cases observed since the start of a given epidemic \emph{wave},
whereas cumulative incidence \textit{per se} is the number of cases
observed since the start of the epidemic.}
Since \cref{eq:exponential,eq:logistic,eq:richards} model growth
\emph{within the fitting window} but not necessarily outside of it,
the output of \code{`9eval_cum_inc`9(time, theta)} must be interpreted
carefully when \code{time} contains elements less than
\code{object$time[object$first]} or greater than
\code{object$time[object$last+`11`1]}, where \code{object}
is the \code{egf_init} object (\code{init} in this example).

The \fun{predict} method for class \code{egf_init} provides a
convenient interface to \fun{eval_cum_inc}.

<<ontario-init-5>>=
predict(init, time = init$time[init$first + 0:10])
@
%
...
namely \code{object$date[`11`1]}. Element \code{cum_inc} is the
value of \code{object$`9cum_inc`9(time, theta = object$theta_init)},
and element \code{int_inc} is just \code{`9diff`9(cum_inc)},
...
the call to \fun{egf_init}. Its inclusion allows the \code{egf_init}
object to be updated using \pkg{stats} function \fun{update}.
This is convenient when the fitting window selected by \fun{egf_init}
is not to your satisfaction, and you want to run \fun{egf_init} again
with a few arguments modified. For details, run \code{?update}.
...
\code{tol} (default is 0).

<<ontario-init-plot-4, eval=FALSE>>=
## Not run

## Highlight all exceptional counts (default)
...
An optional third argument \code{nbdisp_tol} defines a threshold
...
using a Poisson model (see \cref{sec:choosing}). Further optional
arguments will be passed directly to \fun{nlminb}, \fun{nlm}, or
\fun{optim}, depending on \code{method}. Below, we pass \code{init}
to \fun{egf}, accepting the defaults \code{method = `2"nlminb"`2}
and \code{nbdisp_tol}.
...
<<ontario-fit-3>>=
coef(fit) # same as `fit$theta_fit`
coef(fit, log = TRUE) # same as `fit$log_theta_fit`
@
%
...
attach(fit[c("log_theta_fit", "nll", "nll_func", "nll_grad")],
  name = "tmp"
)
...
<<ontario-fit-6>>=
predict(fit, time = init$time[init$first + 0:10])
@
%
...
<<ontario-fit-plot-1, fig.height=7, fig.cap='Result of \\code{`9plot`9(fit, inc)}: \\textbf{[Top]} cumulative incidence (\\code{inc = `2"cumulative"`2}) and \\textbf{[Bottom]} interval incidence (\\code{inc = `2"interval"`2}). Points are observed incidence. Lines are expected incidence conditional on fitted parameter estimates listed at the bottom of the right margin. Incidence is plotted on a logarithmic scale, with zeros plotted directly on the horizontal axis. The shaded rectangle displays the selected fitting window. For interval incidence, a coloured point at time $t_i$ indicates that the observation interval $t_i-t_{i-1}$ differs from the median observation interval (in this case 1 day).'>>=
par(mfrow = c(2, 1)) # create a two-panel plot
