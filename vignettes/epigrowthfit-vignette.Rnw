...
$\tau_\text{inf}$ supported on the integers (\ie $1,2,\ldots$ days).
These are helpful if one wants to use \fun{compute_R0} and
has latent and infectious period distributions, but not the
corresponding generation interval distribution.
