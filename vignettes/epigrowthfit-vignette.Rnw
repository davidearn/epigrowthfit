...
S3 methods. A list of implemented S3 methods can retrieved as
follows.
...
The exported functions apart from \fun{egf_init} and \fun{egf}
enable estimation of additional epidemic parameters, including
the basic reproduction number $\R_0$, from initial epidemic
growth rates $r$ (see \cref{sec:extras}). These functions are
demonstrated in \cref{sec:ontario-extras}.
...
by the initial exponential growth rate $r$ and the distribution of
the disease generation interval $\tau_g$, the time from infection of
a primary case to infection of a secondary case in a chain of
transmission:
%
\begin{linenomath}
\begin{equation}
...
\end{linenomath}
%
where $g$ is the density function of $\tau_g$.
...
\end{enumerate}
%
\cref{eq:R0-general} yields~\cite[Eq 3.6]{WallLips07}
%
\begin{linenomath}
\begin{equation}
\label{eq:R0}
...
$\tau_g \in (\tau_{i-1},\tau_i]$. \pkg{epigrowthfit} function
\fun{compute_R0} evaluates the right-hand side of \cref{eq:R0}
for given $r$, $\tau_i$, and $p_i$, allowing users who have
estimated $r$ using the package's fitting machinery to also
estimate $\R_0$. See \cref{sec:ontario-R0} for an example.
...
period, Svensson~\cite[\S 5.1]{Sven07} gives
%
\begin{linenomath}
\begin{equation}
g = f_\text{lat} * f_\text{wait}\,,\qquad f_\text{wait} = \mathbbm{1}_{(0,\infty)} \frac{1 - F_\text{inf}}{\mathbbm{E}[\tau_\text{inf}]}\,.
\end{equation}
\end{linenomath}
%
Here, $f_\text{wait}$ is the density function of the infectious
...
\fun{pgi} evaluate the density and distribution functions of $\tau_g$
implied by distributions of $\tau_\text{lat}$ and $\tau_\text{inf}$
supported on the integers (\ie $1,2,\ldots$ days). These are helpful
if one wants to use \fun{compute_R0} and has latent and infectious
period distributions, but not the corresponding generation interval
distribution.
...
Once one has compute $\R_0$ from $r$ (see \cref{sec:R0}),
one can compute the expected epidemic final size $Z$ from
$\R_0$. $Z$ is the proportion of the population expected
to be infected over the course of the epidemic, and is
obtained from $\R_0$ as
%
\begin{linenomath}
\begin{equation}
...
any number of infectious stages and arbitrarily distributed
infectious stage durations. \pkg{epigrowthfit} function
\fun{compute_final_size} evaluates the right-hand side of
\cref{eq:final-size-general} for given $\R_0$, $S_0$ and $I_0$.
See \cref{sec:ontario-final-size} for an example.
...
of $\thalf$ is supplied by the time of its peak. Certainly, $K$ should
be no less than the number of cases observed to date and no more than
the population size. When fitting a Richards model, one can initially
assume a logistic model by estimating that $p$ is 1. When including
...
\code{log_theta_hat}.%
\footnote{Recall from \cref{sec:mle} that parameters are all fit on
an unconstrained logarithmic scale. As a result, the negative log
likelihood is defined as a function of the log-transformed parameters.}
Elements \code{nll_fn} and \code{nll_gr} are closures taking a single
numeric argument \code{log_theta} (default is \code{log_theta_hat})
specifying log-transformed parameter values in the order used by
\code{log_theta_hat}. They evaluate the negative log likelihood
function and its gradient with respect to log-transformed parameters
at \code{log_theta}.
...
of \pkg{TMB} function \fun{MakeADFun} and the optimizer from
\pkg{stats} specified by \fun{egf} argument \code{method}
(\fun{nlminb}, \fun{nlm}, or \fun{optim}). These are retained
in the \code{egf} object to allow the user to diagnose poor
fits, as well as warnings and errors issued by the optimizer.
For details, consult the relevant documentation (\eg by running
\code{?TMB::MakeADFun}).
...
@
%
\fun{simulate} returns a list ontaining a copy of argument
...
important epidemic parameters (see \cref{sec:extras})
from initial exponential growth rates $r$. We will
make use of the \code{egf_init} object \code{init}
and \code{egf} object \code{fit} that were obtained
in \cref{sec:ontario-init,sec:ontario-fit}. These
contain our initial and final estimates of $r$
for the Ontario data.
...
exponential growth rates $r$ in units day$^{-1}$, an \code{egf}
object, or an \code{egf_init} object. If \code{x} is a numeric vector,
...
If \code{x} is an \code{egf} object, then it extracts the fitted
value of $r$ from the object and returns the corresponding doubling
time, \ie it returns
\code{`9log`9(`12`1) / object$theta_hat[[`2"r"`2]]}.
Finally, if \code{x} is an \code{egf_init} object, then it extracts
...
or \code{egf} object, in which a value for $r$ is taken from
\code{object$theta0[[`2"r"`2]]} or \code{object$theta_hat[[`2"r"`2]]},
respectively.
...
@
%
Passing \code{breaks} and \code{probs} to \fun{compute_R0},
we obtain values for $\R_0$ as follows.
...
to calculate the expected epidemic final size $Z$ as a function
the basic reproduction number $\R_0$ and initial susceptible and
infected proportions $S_0,I_0 \in [0,1]$. Accordingly, it takes
numeric arguments \code{R0}, \code{S0}, and \code{I0}.
\fun{compute_final_size} is vectorized; its arguments
are recycled up to the length of the longest argument.
