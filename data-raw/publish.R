...
canterbury_raw <- canterbury_raw[c("place", "Date")]
canterbury <- aggregate(seq_len(nrow(canterbury_raw)), canterbury_raw, length)
names(canterbury) <- c("place", "Date", "wills")
o <- do.call(order, canterbury[c("place", "Date")])
...
save(londonparishes, file = "../data/londonparishes.RData")
